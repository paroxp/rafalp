...
interface IExperienceProperties {
...
function Experience(props: IExperienceProperties): JSX.Element {
  const title = `${props.start.format('MMM Do YYYY')} - ${!props.finish ? 'Present' : props.finish.format('MMM Do YYYY')}`;

  return (
    <details data-details={props.nonPrintable ? 'no-print' : ''} open>
      <summary>
        <time dateTime={props.start.format('YYYY-MM-DD')} title={title}>
          {props.start.year()} - {!props.finish ? 'present' : props.finish.year()}
        </time>: {props.title} at <a href={props.organisationURL} target="_blank" rel="external nofollow noopener noreferrer">
          {props.organisation}
...
function Skill(props: ISkillProperties): JSX.Element {
  if (props.minimal) {
    return (
      <div title={props.name} className="minimal">
    ...
    <body>
      ...
      <main className="about">
        <aside>
          ...
          <Skill name="JavaScript" score={75}>
            I cannot really talk about JavaScript without clarifying that I have a strong preference towards
            TypeScript, React and backend code. Although, I've got a lot of experience with jQuery and plugins written
            for it, some time ago I decided, I didn't like it that much and more often found myself trying to avoid
            using it.
          </Skill>
          ...
          <Skill name="Node.js" minimal={true} />
          ...
          <Skill name="PHP" score={75}>
            ...
            with many frameworks, such as Laravel &amp; Lumen, CakePHP, CodeIgniter,  Silex &amp; Cilex, and little
            bits of others, I am able to put together some amazing applications. I go around with Composer and PSRs,
            however I may be a little bit behind, due to my new role, especially with the new PHP 7 stuff.
          </Skill>
          <Skill name="Ruby" minimal={true} />
          <Skill name="Python" minimal={true} />
          <Skill name="HTML" score={90}>
            Not much I could really say here... I try to keep my syntax
            neat and clean, use roles and appropriate tag elements where possible...
            I believe in the potential of HTML5!
          </Skill>
          <Skill name="CSS" score={90}>
            I try to be up to date with CSS trends and functionality. I love SASS and prefer it over Less. I use Susy
            very often and lately, I've started using Bourbon, in order to increase my productivity. Also, mobile-first!
          </Skill>
